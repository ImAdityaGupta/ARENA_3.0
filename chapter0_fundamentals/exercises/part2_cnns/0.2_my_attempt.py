...
class Linear(nn.Module):
    def __init__(self, in_features: int, out_features: int, bias=True):
        ...
        self.weight = (t.rand(out_features, in_features) * 2 * scale) - scale
        self.weight = nn.Parameter(self.weight)

        if self.bias:
            self.bias = nn.Parameter((t.rand(out_features) * 2 * scale) - scale)
        else:
            self.bias = None
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        y = einops.einsum(x, self.weight, '... inf, outf inf -> ... outf')
...
def train(args: SimpleMLPTrainingArgs) -> tuple[list[float], list[float], SimpleMLP]:
    ...
    for epoch in range(args.epochs):
        ...
        for imgs, labels in mnist_testloader:
            ...
            max_likelihood = t.argmax(logits, dim=1)
            count = (max_likelihood == labels).sum().item()
            right += count
            total += imgs.shape[0]
...
class Conv2d(nn.Module):
    def __init__(
        ...
        scale = 1 / np.sqrt(self.in_channels*self.kernel_size*self.kernel_size)
        self.weight = nn.Parameter((self.weight * scale * 2) - scale)
...
# a = Sequential(Linear(100,10), ReLU(), Linear(10,100))
# print(a)
...
class BatchNorm2d(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        """
        if self.training:
            means = einops.reduce(x, 'b c h w -> c', 'mean')
            variances = einops.reduce(x, 'b c h w -> c', t.var)

            self.running_mean = (1-self.momentum)*self.running_mean + self.momentum*means
            self.running_var = (1-self.momentum)*self.running_var + self.momentum*variances
            self.num_batches_tracked += 1
        else:
            means = self.running_mean
            variances = self.running_var


        means = einops.rearrange(means, 'c -> 1 c 1 1')
        variances = einops.rearrange(variances, 'c -> 1 c 1 1')

        weights = einops.rearrange(self.weight, 'c -> 1 c 1 1')
        biases = einops.rearrange(self.bias, 'c -> 1 c 1 1')

        return ((x-means) / t.sqrt(variances+self.eps))*weights + biases
...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        """
        left_side = self.left(x)
        right_side = self.right(x)

        return self.relu(left_side+right_side)



with t.inference_mode():   # or: with t.no_grad():
...
class ResNet34(nn.Module):
    def __init__(
        ...
        self.blocks = Sequential(
            ...
            ReLU(),
            # Linear(out_feats0, self.out_features_per_group[-1]),
            MaxPool2d(3, 2),
            BlockGroup(n_blocks_per_group[0], out_feats0, out_features_per_group[0], first_strides_per_group[0]),
            *[BlockGroup(n_blocks_per_group[n+1], out_features_per_group[n], out_features_per_group[n+1], first_strides_per_group[n+1]) for n, i in enumerate(n_blocks_per_group[1:])],
            AveragePool(),
            Linear(self.out_features_per_group[-1], 1000)
        )

        # for n, i in enumerate(n_blocks_per_group[1:])
