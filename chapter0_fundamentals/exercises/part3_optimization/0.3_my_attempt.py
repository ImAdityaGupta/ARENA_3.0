...
# optimizer_list = [
#     (optim.SGD, {"lr": 0.02, "momentum": 0.999}),
#     (optim.SGD, {"lr": 0.02, "momentum": 0.99}),
#     (optim.SGD, {"lr": 0.03, "momentum": 0.9999}),
# ]
...
class RMSprop:
    ...
    def step(self) -> None:
        for n, p in enumerate(self.params):
            ...
            if self.lmda != 0:
                g = g + self.lmda*p
            self.v[n] = self.alpha*self.v[n] + (1-self.alpha)*g*g
            g = g / (t.sqrt(self.v[n])+self.eps)
            if self.mu != 0:
                self.b[n] = self.mu*self.b[n] + g
                g = self.b[n]
            p.copy_(p - self.lr*g)
...
class ResNetFinetuner:
    ...
    def pre_training_setup(self):
        ...
        self.train_loader = DataLoader(self.trainset, batch_size=self.args.batch_size, shuffle=True, num_workers=4)
        self.test_loader = DataLoader(self.testset, batch_size=self.args.batch_size, shuffle=False, num_workers=4)
...
sweep_config = dict(
    ...
    parameters = dict(
        ...
        do_weight_decay = dict(values=[True, False]),
...
def update_args(
    ...
    if sampled_parameters["do_weight_decay"]:
        args.weight_decay = sampled_parameters["weight_decay_value"]
    else:
